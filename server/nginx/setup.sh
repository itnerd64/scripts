...
sed -i "s/DOMAIN/$DOMAIN/g" http.conf
sed -i "s/DOMAIN/$DOMAIN/g" https.conf
sed -i "s/DOMAIN/$DOMAIN/g" webroot.conf
sed -i "s/DOMAINSHOULDBECHANGED/$DOMAIN/g" https.sh
sed -i "s/EMAILSHOULDBECHANGED/$EMAIL/g" https.sh

# Move https related files to root
printf "\nMoving https related files to root...\n"
sudo mv https.sh /root/Server/
...
< webroot.conf sudo tee /etc/nginx/includes/letsencrypt-webroot >/dev/null
